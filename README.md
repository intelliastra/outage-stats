...
| `统计材料/运行脚本/statistics_priority.py` | 统计表用户与线路互斥优先级和去重 |
| `统计材料/运行脚本/report_comparison.py` | 本次与上次成功日报及旧表实际数值的变化对比 |
| `统计材料/运行脚本/external_damage_filter.py` | 外力原因代码 513/514/515 前缀过滤 |
...
- 统计表内频繁停电高于预警；用户按用户编码、线路按馈线编码只归一个最高优先级子列。频繁内部为年>5、60天>3、预安排>3；预警内部为30天2、50天3、年4–5。2025与2026表按同口径生成。
- 日常日报与上次成功日报比较：既对比旧统计表实际 B–U 数值，也对比清单成员、次数及分类；减少单独写入摘要。“变化明细”Sheet仅列次数减少或次数相同但类型改变的对象，减少行标黄，显示全部命中类型并附备注；消失对象仍可定位。正式统计表继续按最高优先级互斥计数。同日期范围再次运行生成新版文件，保留旧版。
- 外力页面需上传与日常页内容相同的原始 Excel。脚本确认 AE 列为“责任原因代码”，再排除 513*、514*、515* 行；日常统计不排除这些行。年度表11重大事件日配置仍是独立剔除规则。
- 详细公式、列映射和边界测试见 [统计表统计口径与条件](docs/统计表统计口径与条件.md)。
