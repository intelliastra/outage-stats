#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""历史区间统计 — 基于 v1.0.8 内核，专用于指定日期区间的停电可靠性统计。

Version: 1.0.0 — 通过 --period-start / --period-end 指定统计区间，
输出落盘至 output/historical/<日期范围>/，与日常统计 output/daily/ 物理隔离。
"""
...
# --- 输入路径（历史区间统计口径） ---
NEWDATA_FOLDER       = _os.path.join(_SCRIPT_DIR, "input", "Newdata", "historical")
...
# --- 输出根目录（历史区间统计 → output/historical/<日期范围>/） ---
OUTPUT_BASE_DIR      = _os.path.join(_SCRIPT_DIR, "output", "historical")
# 历史处理结果：Newdata 日期窗口内整段替换，窗口外保留
EXISTINGDATA_FOLDER  = OUTPUT_BASE_DIR

# --- 以下留空则自动命名（推荐留空） ---
RESULT_OUTPUT_PATH   = ""    # 留空 → output/historical/<日期范围>/<日期范围>停电用户_处理结果.xlsx
PUBLISH_OUTPUT_PATH  = ""    # 留空 → output/historical/<日期范围>/<日期范围>停电用户_处理结果_发出版.xlsx
STATS_OUTPUT_PATH    = ""    # 留空 → output/historical/<日期范围>/<日期范围>统计表.xlsx
